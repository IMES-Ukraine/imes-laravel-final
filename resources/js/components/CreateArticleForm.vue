<template>
	...
	<div class="main-articles">
        ...
        <a href="" @click="$router.push('/project/content')" class="article-edit__close" aria-label="закрити" title="закрити"></a>
        <h4><center>Статья</center></h4>
        <div class="card-body">
            <div class="row mb-4">
...
export default {
    ...
    mounted() {
        axios.get(process.env.VUE_APP_API_URI + `/blog?count=12&type=1`).then(
                        response => {
                            this.recommended = response.data.data.data
                        })

        axios.get(process.env.VUE_APP_API_URI + `/users?count=12`).then(
                        response => {
